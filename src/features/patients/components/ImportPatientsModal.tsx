...
export function ImportPatientsModal({ isOpen, onClose, onImported }: Props) {
  ...
  // Mỗi lần popup chuyển từ đóng -> mở, reset trạng thái cũ ngay trong lúc
  // render (thay vì trong effect) để tránh setState đồng bộ trong effect body.
  ...
  // Nạp danh sách hồ sơ từ HIS mỗi lần mở popup.
  useEffect(() => {
    if (!isOpen) return
    let active = true
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { message?: string | string[] } } })?.response?.data
        ?.message
      setImportError(Array.isArray(msg) ? msg.join(', ') : msg || 'Có lỗi xảy ra khi import hồ sơ.')
    } finally {
      setImporting(false)
    }
  }
          ...
          {result && (
            <div className="space-y-3">
              <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
                ...
                {result.skipped > 0 && (
                  <>
                    , <b>{result.skipped}</b> đã tồn tại (bỏ qua)
                  </>
                )}
                {result.failed > 0 && (
                  <>
                    , <b>{result.failed}</b> lỗi
                  </>
                )}
                .
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {result.results.map((r) => (
                      <tr key={r.recordId}>
                        ...
                        <td className="px-4 py-2">{r.roomBed ?? '--'}</td>
