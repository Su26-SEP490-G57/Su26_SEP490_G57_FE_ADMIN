...
// Danh sach benh nhan gom theo phong — moi phong la 1 hang cuon ngang cac
// PatientAnalyticsCard (chon 1 de xem chi tiet ben duoi).
      ...
      {groups.map((group) => (
        <div key={group.room}>
          <div className="mb-2 flex items-center gap-2">
            <h4 className="text-sm font-bold text-slate-700">Phong {group.room}</h4>
            <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-600">
              {group.patients.length} nguoi benh
            </span>
          </div>
