...
const PRIORITY_PATIENTS = [
  {
    ...
    pathway: 'Dạ dày',
    level: 'RED',
    symptoms: 'Buồn nôn nhiều, chướng bụng',
    time: '08:20',
    nurse: 'ĐD. Nguyễn Thị Hoa',
  },
  {
    id: 'BN008',
    name: 'Trần Thị B',
    pod: 'POD 1',
    pathway: 'Đại trực tràng',
    level: 'RED',
    symptoms: 'Nôn 2 lần, chưa trung tiện',
    time: '07:55',
    nurse: 'ĐD. Nguyễn Thị Hoa',
  },
  {
    ...
    pathway: 'Dạ dày',
    level: 'YELLOW',
    symptoms: 'Ăn uống kém',
    time: '07:40',
    nurse: 'ĐD. Trần Văn Nam',
  },
]

const ALERTS = [
  {
    time: '12/05/2025 08:20',
    id: 'BN001',
    content: 'Buồn nôn nhiều, chướng bụng nhiều',
    pod: 'POD 2 - Sáng',
    level: 'RED',
    status: 'Chưa xử lý',
    nurse: '-',
  ...
  {
    time: '12/05/2025 07:55',
    id: 'BN008',
    content: 'Nôn 2 lần, chưa trung tiện',
    pod: 'POD 1 - Sáng',
    level: 'RED',
    status: 'Chưa xử lý',
    nurse: '-',
  ...
  {
    time: '11/05/2025 15:10',
    id: 'BN015',
    content: 'Ăn uống kém, chướng bụng',
    pod: 'POD 2 - Chiều',
    level: 'YELLOW',
    status: 'Đã xử lý',
    nurse: 'ĐD. Trần Văn Nam',
...
const donutData = {
  datasets: [
    { data: [15, 6, 3], backgroundColor: ['#22c55e', '#fbbf24', '#ef4444'], borderWidth: 0 },
  ],
}
const donutOptions = {
  responsive: true,
...
const barData = {
  ...
  datasets: [
    {
      data: [4, 5, 6, 4, 5],
      backgroundColor: ['#3b82f6', '#14b8a6', '#6366f1', '#eab308', '#ef4444'],
      borderRadius: 4,
      barThickness: 16,
    },
  ],
...
function LevelBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${map[level] ?? 'bg-slate-200 text-slate-600'}`}
    >
      {level}
    </span>
  )
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-end gap-4">
        <div className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-4 py-2 shadow-sm">
          ...
          <span className="material-symbols-outlined cursor-pointer text-[14px] text-slate-400">
            sync
          </span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
        ...
        <div className="flex items-center gap-4 rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
          ...
          <div className="flex-1">
            ...
            <div className="mt-1 h-1.5 w-full rounded-full bg-slate-100">
              <div className="h-1.5 rounded-full bg-red-500" style={{ width: `${pct(red)}%` }} />
            </div>
          </div>
        </div>
        {/* Alert */}
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {PRIORITY_PATIENTS.map((p) => (
                  <tr key={p.id}>
                    <td
                      className={`px-4 py-3 font-bold ${p.level === 'RED' ? 'text-red-500' : 'text-yellow-500'}`}
                    >
                      {p.id}
                    </td>
        ...
        <div className="col-span-4 grid grid-cols-2 gap-4">
          ...
          <div className="flex flex-col items-center rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="w-full space-y-1 px-2">
              {[
                { color: 'bg-green-500', label: 'GREEN', val: '15 (62.5%)' },
                { color: 'bg-yellow-400', label: 'YELLOW', val: '6 (25.0%)' },
                { color: 'bg-red-500', label: 'RED', val: '3 (12.5%)' },
              ].map((item) => (
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex items-center border-b border-slate-200 px-6">
          ...
          <div className="flex h-full gap-8 text-xs font-medium text-slate-500">
            {[
              ...
              <button
                key={tab}
                className={`h-full py-4 hover:text-blue-600 ${i === 0 ? 'border-b-2 border-blue-600 text-blue-600' : ''}`}
              >
                {tab}
              </button>
            ))}
        ...
        <div className="grid grid-cols-12 gap-6 p-6">
          ...
          <div className="col-span-3">
            <h4 className="mb-4 flex items-center gap-2 text-xs font-bold text-blue-600">
              Thông tin chung
            </h4>
            <div className="space-y-3 rounded-lg bg-slate-50 p-4 text-xs">
              {[
                ...
                ['Tuổi', '58'],
                ['Giới tính', 'Nam'],
                ['Ngày phẫu thuật', '10/05/2025'],
                ['Pathway', 'Gastric pathway'],
                ['BMI', '22.1'],
                ['Điều dưỡng phụ trách', 'ĐD. Nguyễn Thị Hoa'],
                ['Bác sĩ phụ trách', 'BS. Trần Văn Nam'],
              ].map(([label, value]) => (
                  ...
                  {POD_ROWS.map((row) => (
                    <tr key={row.pod} className={row.current ? 'bg-blue-50' : ''}>
                      <td className="p-2 font-bold">
                        ...
                        <br />
                        <span
                          className={`font-normal ${row.current ? 'text-red-500' : 'text-slate-400'}`}
                        >
                          {row.date}
                        </span>
                      </td>
                      <td
                        className={`p-2 ${row.current ? 'bg-red-50 font-bold text-red-600' : ''}`}
                      >
                        {row.morning ?? '-'}
                      </td>
                      <td className="p-2">
                        {row.afternoon ??
                          (row.current ? (
                            <div className="mx-auto flex h-6 w-6 items-center justify-center rounded-full bg-red-600 text-white">
                      ...
                      <td className={`p-2 font-bold ${row.level === 'RED' ? 'text-red-600' : ''}`}>
                        {row.total ?? '-'}
                      </td>
                      <td className="p-2">{row.level ? <LevelBadge level={row.level} /> : '-'}</td>
                      <td
                        className={`p-2 font-medium ${row.status === 'Đã đánh giá' ? 'text-green-600' : row.status === 'Đang theo dõi' ? 'text-red-600' : 'text-slate-400'}`}
                      >
                        {row.status}
                      </td>
                    </tr>
          ...
          {/* Line Chart */}
          <div className="col-span-3">
            <h4 className="mb-4 text-xs font-bold text-blue-600">
              Biểu đồ triệu chứng (POD 1 → POD 2 sáng)
            </h4>
            ...
            <div className="mt-4 flex flex-wrap gap-2 text-[8px] text-slate-500">
              {[
                ['bg-red-500', 'Buồn nôn'],
                ['bg-yellow-500', 'Nôn'],
                ['bg-purple-500', 'Chướng bụng'],
                ['bg-blue-500', 'Ăn uống'],
                ['bg-teal-500', 'Trung tiện'],
              ].map(([color, label]) => (
            ...
            <p className="mt-2 text-[9px] text-slate-400">
              Giải thích: 0 = Không, 1 = Nhẹ, 2 = Vừa, 3 = Nhiều
            </p>
          </div>
          ...
          <div className="col-span-2 space-y-4">
            <div className="rounded-lg border border-red-100 bg-red-50 p-3">
              ...
              <button className="mt-2 w-full text-[10px] font-bold text-blue-600 hover:underline">
                Xem chi tiết alert
              </button>
            </div>
            <div className="space-y-2">
              ...
              <button className="flex w-full items-center justify-center gap-2 rounded bg-green-600 py-1.5 text-[10px] font-bold text-white">
                <span className="material-symbols-outlined text-[14px]">play_circle</span> Tiếp tục
                theo dõi
              </button>
              <button className="flex w-full items-center justify-center gap-2 rounded bg-yellow-500 py-1.5 text-[10px] font-bold text-white">
                <span className="material-symbols-outlined text-[14px]">pause_circle</span> Giữ POD
                hiện tại
              </button>
              <button className="flex w-full items-center justify-center gap-2 rounded bg-red-600 py-1.5 text-[10px] font-bold text-white">
                <span className="material-symbols-outlined text-[14px]">medical_services</span> Báo
                bác sĩ
              </button>
              <button className="flex w-full items-center justify-center gap-2 rounded border border-slate-300 py-1.5 text-[10px] font-bold text-slate-700">
                <span className="material-symbols-outlined text-[14px]">edit_note</span> Ghi chú
                điều dưỡng
              </button>
            </div>
          </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {ALERTS.map((a, i) => (
                <tr key={i}>
                  ...
                  <td
                    className={`px-6 py-4 font-bold ${a.status === 'Chưa xử lý' ? 'text-red-500' : 'text-green-600'}`}
                  >
                    {a.status}
                  </td>
                  ...
                  <td className="px-6 py-4 text-center">
                    {a.status === 'Chưa xử lý' ? (
                      <button className="rounded border border-red-500 px-3 py-1 text-[10px] font-bold text-red-500 hover:bg-red-50">
                        Xử lý
                      </button>
                    ) : (
