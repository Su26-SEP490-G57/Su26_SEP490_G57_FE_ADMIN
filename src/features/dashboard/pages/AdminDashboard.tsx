...
const USERS = [
  {
    initials: 'AN',
    name: 'Nguyễn Thị An',
    email: 'an.nguyen@hospital.org',
    role: 'Điều dưỡng trưởng',
    lastActivity: '2 phút trước',
    status: 'Đang hoạt động',
    statusBg: 'bg-[#86f2e4]/30 text-[#006a61]',
    dotColor: 'bg-[#006a61]',
    avatarBg: 'bg-[#d8e2ff] text-[#00459a]',
  },
  {
    initials: 'BM',
    name: 'Trần Văn Minh',
    email: 'minh.tran@hospital.org',
    role: 'Dược sĩ trưởng',
    lastActivity: '14 phút trước',
    status: 'Đang hoạt động',
    statusBg: 'bg-[#86f2e4]/30 text-[#006a61]',
    dotColor: 'bg-[#006a61]',
    avatarBg: 'bg-[#89f5e7] text-[#006a61]',
  },
  {
    initials: 'LH',
    name: 'Lê Thị Hương',
    email: 'huong.le@hospital.org',
    role: 'Quản trị CNTT',
    lastActivity: '2 ngày trước',
    status: 'Ngoại tuyến',
    statusBg: 'bg-[#e3e1ec]/30 text-[#727785]',
    dotColor: 'bg-[#727785]',
    avatarBg: 'bg-[#e3e1ec] text-[#424753]',
  },
]
...
const AUDIT_LOGS = [
  {
    type: 'Bảo mật',
    typeColor: 'text-[#00459a]',
    borderColor: 'border-[#00459a]',
    message: 'Gán quyền quản trị cho huong.le',
    time: 'Hôm nay, 09:12',
  },
  {
    type: 'Hệ thống',
    typeColor: 'text-[#424753]',
    borderColor: 'border-[#c2c6d5]',
    message: 'Sao lưu tự động thành công (S3-EU-1)',
    time: 'Hôm nay, 03:00',
  },
  {
    type: 'Cảnh báo',
    typeColor: 'text-[#ba1a1a]',
    borderColor: 'border-[#ba1a1a]',
    message: 'Đăng nhập thất bại: IP 192.168.1.1',
    time: 'Hôm qua, 23:45',
  },
  {
    type: 'Phác đồ',
    typeColor: 'text-[#424753]',
    borderColor: 'border-[#c2c6d5]',
    message: 'Cập nhật Chỉ thị An toàn Bệnh nhân v4.2',
    time: 'Hôm qua, 16:30',
  },
]
...
const NOTIFICATION_CHANNELS = [
  {
    ...
    sub: 'Báo cáo khẩn và tóm tắt hệ thống',
    defaultOn: true,
  },
  {
    icon: 'sms',
    label: 'Cầu nối SMS',
    sub: 'Chuyển tiếp cảnh báo sinh lý nghiêm trọng',
    defaultOn: false,
  },
  {
    icon: 'terminal',
    label: 'Tích hợp Webhook',
    sub: 'Chuyển nhật ký đến công cụ SIEM bên ngoài',
    defaultOn: true,
  },
]

export function AdminDashboard() {
  const [toggles, setToggles] = useState(NOTIFICATION_CHANNELS.map((c) => c.defaultOn))
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
        <div className="flex flex-col justify-between rounded-2xl border border-[#c2c6d5] bg-white p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
          <div className="flex items-start justify-between">
            ...
            <span className="rounded-full bg-[#86f2e4]/30 px-2 py-0.5 text-[10px] font-semibold text-[#006a61]">
              +12% so với năm trước
            </span>
          </div>
          <div className="mt-4">
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-[#424753]">
        ...
        <div className="flex flex-col justify-between rounded-2xl border border-[#c2c6d5] bg-white p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
          <div className="flex items-start justify-between">
            ...
            <span className="rounded-full bg-[#ffdad6]/30 px-2 py-0.5 text-[10px] font-semibold text-[#ba1a1a]">
              -3% trực tiếp
            </span>
          </div>
          <div className="mt-4">
          ...
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-[#424753]">
                Giám sát thông lượng và phân bổ nguồn lực theo thời gian thực
              </p>
            </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-[#c2c6d5] bg-white shadow-sm lg:col-span-3">
          ...
          <table className="w-full text-left">
            <thead className="bg-[#f4f2fd]/50">
              <tr>
                {['Thành viên', 'Vai trò', 'Hoạt động gần nhất', 'Trạng thái', 'Thao tác'].map(
                  (h) => (
                    <th
                      key={h}
                      className="px-6 py-4 text-[10px] font-bold uppercase tracking-wider text-[#424753]"
                    >
                      {h}
                    </th>
                  ),
                ...
                <tr key={u.email} className="transition-colors hover:bg-[#f4f2fd]/30">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <div
                        className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-bold ${u.avatarBg}`}
                      >
                        {u.initials}
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold ${u.statusBg}`}
                    >
              ...
              <div key={p.module} className="grid grid-cols-4 items-center gap-4 py-1">
                ...
                {[p.admin, p.physician, p.nurse].map((allowed, i) => (
                  <div key={i} className="flex justify-center">
                    <span
                      className={`material-symbols-outlined text-[20px] ${allowed ? 'text-[#006a61]' : 'text-[#c2c6d5]'}`}
                    >
