  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setErrorNotification(
        translateError(
          err,
          'Không thể xóa câu hỏi này do đã được sử dụng trong các lượt đánh giá.',
        ),
      )
    }
  }
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500">
              Quản lý câu hỏi khảo sát triệu chứng hàng ngày cho bệnh nhân ERAS, chỉnh sửa nội dung
              và điều chỉnh trọng số điểm số (score weights)
            </p>
          </div>
