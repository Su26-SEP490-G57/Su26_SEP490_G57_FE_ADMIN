...
interface LocalOption {
  optionId?: number // optional if newly added locally
  optionText: string
  scoreValue: number
  isNew?: boolean
  isDeleted?: boolean
...
export function QuestionDetailView({ questionId, onBack }: QuestionDetailViewProps) {
  ...
  // Populate local state when question data is fetched
  useEffect(() => {
    if (question) {
      setQuestionText(question.questionText)
      setOrderNumber(question.orderNumber ?? 1)
      setIsDefault(question.isDefault)
      setOptions(
  ...
  const handleRemoveOption = (index: number) => {
    ...
    if (target.isNew) {
      ...
      // Mark as deleted
      setOptions((prev) =>
        prev.map((opt, idx) => (idx === index ? { ...opt, isDeleted: true } : opt)),
      )
    }
  }
  ...
  const handleRestoreOption = (index: number) => {
    setOptions((prev) =>
      prev.map((opt, idx) => (idx === index ? { ...opt, isDeleted: false } : opt)),
    )
  }
  ...
  const handleSaveChanges = async () => {
    ...
    if (activeOptions.length === 0) {
      setSubmitError('Câu hỏi phải có ít nhất 1 lựa chọn trả lời.')
      return
    }

    for (const opt of activeOptions) {
      if (!opt.optionText.trim()) {
        setSubmitError('Nội dung lựa chọn không được để trống.')
        return
      }
      if (opt.scoreValue < 0) {
        setSubmitError('Trọng số điểm phải lớn hơn hoặc bằng 0.')
        return
      }
    }
    ...
    try {
      ...
      for (const opt of options.filter((o) => o.isDeleted && o.optionId)) {
      ...
      for (const opt of options.filter((o) => !o.isDeleted)) {
        if (opt.isNew || !opt.optionId) {
          await addOptionMutation.mutateAsync({
            questionId: question.questionId,
            data: {
              optionText: opt.optionText.trim(),
              scoreValue: Number(opt.scoreValue),
            },
          })
        } else {
                    ...
                    >
                      ...
                      <div className="flex-1 space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleUpdateOptionField(idx, 'optionText', e.target.value)
                          }
                      ...
                      <div className="w-full sm:w-44 space-y-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              handleUpdateOptionField(idx, 'scoreValue', score + 1)
                            }
                            className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-100 active:scale-95"
                          >
                            <span className="material-symbols-outlined text-[16px]">add</span>
